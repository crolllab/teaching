...
server <- function(input, output) {
  ...
  loci_react <- reactive({
    ...
    s  <- pmin(s, 0.99)    # cap: lethal mutations rare in standing variation
    s  <- pmax(s, 1e-4)   # floor: avoid effectively neutral s = 0
  ...
  output$loadPlot <- renderPlot({
    ...
    ggplot() +
      ...
      labs(title = paste0("Genetic load  (Ne = ", input$Ne,
           ...
           x = "Generation", y = "Genetic load  (L = 1 \u2212 W\u0305)") +
  ...
  output$fatePlot <- renderPlot({
    ...
    fate_df <- data.frame(
      s           = loci$s,
      prop_purged = colMeans(q_final < 0.01),
      prop_fixed  = colMeans(q_final > 0.99),
      prop_segreg = colMeans(q_final > 0.01 & q_final < 0.99)
    ) %>%
      pivot_longer(starts_with("prop_"), names_to = "outcome", values_to = "proportion") %>%
      mutate(outcome = recode(outcome,
        prop_purged = "Purged  (q < 0.01)",
        prop_fixed  = "Fixed  (q > 0.99)",
    ...
    ggplot(fate_df, aes(x = s, y = proportion, colour = outcome)) +
      ...
      scale_colour_manual(values = c(
        "Purged  (q < 0.01)" = "#4DAF4A",
        "Fixed  (q > 0.99)"  = "#E41A1C",
        "Segregating"        = "#377EB8")) +
      labs(title = paste0("Locus fate by selection coefficient  (Ne = ", input$Ne, ")"),
           x = "Selection coefficient (s, log scale)", y = "Proportion of replicates",
           colour = "Outcome") +
      theme_classic(base_size = 13) +
  ...
  output$summaryTable <- renderTable({
    ...
    data.frame(
      Statistic = c(
        ...
        "Mean proportion of loci purged (q < 0.01)",
        "Mean proportion of loci fixed  (q > 0.99)",
      ...
      Value = round(c(
        ...
        mean(q_final < 0.01),
        mean(q_final > 0.99),
        mean(q_final > 0.01 & q_final < 0.99)
      ), 4)
    )
  })
}
