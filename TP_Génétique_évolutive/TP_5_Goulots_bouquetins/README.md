...
# notez que vous pouvez aussi assigner les individus
# selon la colonne "Reintroduction_History") ou selon la colonne "Reintroduction_Genealogy"
```
...
# Visualisez les Fst et sauver le graphique
```
...
Q8: Quelle est l'impact d'un goulot d'étranglement sur le niveau de l'hétérozygotie?
...
# rajouter l'information sur l'histoire de la population
...
Q9: Visualisez l'évolution de l'hétérozygotie au cours des réintroductions à l'aide du code ci-dessus. Est-ce que les changements correspondent à vos attentes? Explications possibles des résultats inattendus?

## L'effet des goulots d'étranglement sur le F<sub>IS</sub>

Q10: Donnez la définition du F<sub>IS</sub> et son interprétation.

Ci-dessous, nous allons utiliser le package `hierfstat` pour calculer une série de paramètres de génétique de populations (y inclut le F<sub>IS</sub>).

```
# Calculs d'un grand nombre de statistiques par population et locus (y inclut les Fst).
...
Q11: Quelles sont les statistiques incluses dans l'objet `pop.stats`?

Générez un résumé des données dans un seul data.frame et visualisez les F<sub>IS</sub>
...
Visualisation des fréquences alléliques à un locus aléatoire
...
Une analyse plus complète est bien nécessaire. Voilà une option de code pour visualiser les fréquences alléliques à 10 loci choisi aléatoirement

```
# Création d'un data.frame contenant l'ensemble des fréquences alléliques

# matrice contenant des fréquences pour REF et ALT à chaque locus
full.freq.matrix <- do.call(rbind, pop.stats$pop.freq)

# sélection des fréquences REF uniquement, élimination des rownames car toujours 0
...
Q15: Essayez une série de loci aléatoirement choisis (relancer la sélection de 10 loci ci-dessus). Sélectionnez un graphique qui vous paraît explicative et puis interprétez l'évolution des fréquences alléliques en fonction de l'histoire évolutive des  populations (introduction primaire, secondaire, admixture, etc.)

Q16: Après avoir fait toutes ces analyses: Est-ce qu'il vous semble que les bouquetins sont en danger? Et puis, si vous étiez en charge de planifier des translocations d'invidus, vous les feriez entre quelles populations? Argumentez en s'appuyant sur les résultats que vous avez obtenus.